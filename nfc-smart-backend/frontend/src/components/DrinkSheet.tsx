...
export function DrinkSheet({
  ...
  const info = getDrinkInfo(group.pairing.label);
          ...
          >
            ...
            <div className="mt-3 flex flex-col gap-2">
              {group.dishes.map((dish, idx) => (
                ...
                >
                  <span className="font-display text-[16px] font-medium text-text">{dish.name}</span>
                  <span className="shrink-0 text-[13px] tabular-nums text-text-muted">
                    {euro(group.pairing.price)}
                  </span>
                  <button
                    type="button"
                    onClick={() => onTake(group.keys[idx])}
                    className="shrink-0 bg-accent px-3.5 py-2 text-[11px] font-medium uppercase tracking-[0.18em] text-accent-ink active:scale-95"
                  >
                    {t.addShort}
                  </button>
                </div>
